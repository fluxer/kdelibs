...
public:
    ...
    {
        QList<QLabel*> wl = view->widget()->findChildren<QLabel*>(QLatin1String("KHTMLAccessKey"));
        foreach(QLabel* w, wl) {
...
void KHTMLView::mouseMoveEvent( QMouseEvent * _mouse )
{
    ...
    if (r && r->isWidget()) {
        ...
        if (kw && kw->m_kwp->isRedirected())
            ...
            QList<QWidget*> wl = le->findChildren<QWidget *>(QLatin1String("KLineEditButton"));
...
QMap< ElementImpl*, QChar > KHTMLView::buildFallbackAccessKeys() const
{
    ...
    for( NodeImpl* n = m_part->xmlDocImpl();
        ...
        if( n->isElementNode()) {
            ...
            switch( element->id()) {
                case ID_A:
                    url = element->getAttribute(ATTR_HREF).parsedUrl().string();
