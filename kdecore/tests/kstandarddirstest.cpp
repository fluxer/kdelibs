...
#define EXT ""
#define KIOSLAVE "katana/libexec/kioslave"
...
void KStandarddirsTest::testFindExe()
{
    ...
    QVERIFY( lnusertemp.endsWith( "lib" KDELIBSUFF "/katana/libexec/lnusertemp" EXT, PATH_SENSITIVITY ) );

    // locate("exe") with a result in libexec
    const QString locateExeResult = KGlobal::dirs()->locate("exe", "lnusertemp");
    QVERIFY(locateExeResult.endsWith("lib" KDELIBSUFF "/katana/libexec/lnusertemp" EXT, PATH_SENSITIVITY));
