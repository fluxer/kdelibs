...
void KStringHandlerTest::naturalCompare()
{
    ...
    QCOMPARE(KStringHandler::naturalCompare("A", "a", Qt::CaseSensitive) < 0, QString::localeAwareCompare("A", "a") < 0);
...
void KStringHandlerTest::preProcessWrap_data()
{
    ...
    QTest::newRow("underscores") << "foo_bar_baz"
               << QString(QString("foo_") + zwsp + "bar_" + zwsp + "baz");

    // Should insert a ZWSP after each '-'
    QTest::newRow("hyphens") << "foo-bar-baz"
                << QString(QString("foo-") + zwsp + "bar-" + zwsp + "baz");
    ...
    QTest::newRow("single quote") << "foo'bar"<< QString(QString("foo") + wj +"'bar");
}
